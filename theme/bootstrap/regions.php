			...
			<table class="table table-striped">
				...
				foreach($regions as $region_name => $region) {
					...
					if($this->permission_handler->has_permission('update', 'region', $region['region_id'])) {
						echo " <a href=\"".BASE_PATH."/admin/new_edit/region/".$region['region_id']."\"><i class=\"fa fa-pencil\"></i></a>";
						$auth = true;
					}
					if($this->permission_handler->has_permission('delete', 'region', $region['region_id'])) {
						echo " <a href=\"".BASE_PATH."/admin/delete/region/".$region['region_id']."\" onclick=\"return confirm('Do you really want to delete this region?')\"><i class=\"fa fa-trash\"></i></a></td>";
						$auth = true;
					}
					if(!$auth) {
						echo "<i class=\"fa fa-lock\" title=\"You have no permissions to edit this region\"></i>";
					}
