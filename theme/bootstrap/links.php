			...
			<table class="table table-striped">
				...
				foreach($links as $link) {
					...
					if($this->permission_handler->has_permission('update', 'link', $link_id)) {
						echo " <a href=\"".BASE_PATH."/admin/new_edit/link/".$link_id."\"  title=\"Edit this link\"><i class=\"fa fa-pencil\"></i></a>";
						$auth = true;
					}
					if($this->permission_handler->has_permission('delete', 'link', $link_id)) {
						echo " <a href=\"".BASE_PATH."/admin/delete/link/".$link_id."\" title=\"Delete this link\" onclick=\"return confirm('Do you really want to delete this menu link?')\"><i class=\"fa fa-trash\"></i></a></td>";
						$auth = true;
					}
					if(!$auth) {
						echo "<i class=\"fa fa-lock\" title=\"You have no permissions to manage this link\"></i>";
					}
